...
module.exports = {
    ...
    findPartContent:    findPartContent,
    fromParts:          fromParts,
};
...
function getParts( component, partName ){
    ...
    for ( var i=0,len=component.parts.length; i<len; i++ ){
        if ( component.parts[i].partName === partName ){
            parts.push( component.parts[i] );
        }
    }
...
function findParts( component, partNames ){

    var parts =         [];

    for ( var i=0, len=component.parts.length; i<len; i++ ){
        if ( -1 !== partNames.indexOf( component.parts[i].partName )){
            parts.push( component.parts[i] );
        }
    }
...
function findPartContent( component, partNames ){

    var parts =         findParts( component, partNames );
    return parts[0] && parts[0].content || "";
}///
